        ...
        if (ctx != null) {
            try {
                ...
                if (servletContext != null) {
                    lookupString = "java:global" + servletContext.getContextPath() + "/" + ProductosFacade.class.getSimpleName();
                } else {
                    lookupString = "java:global/" + ProductosFacade.class.getSimpleName();
                }
                ejbFacade = (ProductosFacade) ctx.lookup(lookupString);
            } catch (NamingException ex) {
                Logger.getLogger(ProductosConverter.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
