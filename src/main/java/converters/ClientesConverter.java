...
public class ClientesConverter implements Converter {
    ...
    public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
        ...
        try {
            ...
            Logger.getLogger(ClientesConverter.class.getName()).log(Level.SEVERE, null, ex);
        }
        if (ctx != null) {
            try {
                String lookupString;
                if (servletContext != null) {
                    lookupString = "java:global" + servletContext.getContextPath() + "/" + ClientesFacade.class.getSimpleName();
                } else {
                    lookupString = "java:global/" + ClientesFacade.class.getSimpleName();
                }
                ejbFacade = (ClientesFacade) ctx.lookup(lookupString);
            } catch (NamingException ex) {
                Logger.getLogger(ClientesConverter.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
